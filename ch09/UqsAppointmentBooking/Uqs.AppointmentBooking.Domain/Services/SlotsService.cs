...
public class SlotsService : ISlotsService
{
    ...
    public async Task<Slots> GetAvailableSlotsForEmployee(int serviceId, int employeeId)
    {
        ...
        foreach (var shift in shifts)
        {
            ...
            var potentialAppointmentEnd = potentialAppointmentStart.AddMinutes(
                service.AppointmentTimeSpanInMin
            );

            for (
                int increment = 0;
                potentialAppointmentEnd <= shift.Ending;
                increment += _settings.RoundUpInMin
            )
            {
                potentialAppointmentStart = shift.Starting.AddMinutes(increment);
                potentialAppointmentEnd = potentialAppointmentStart.AddMinutes(
                    service.AppointmentTimeSpanInMin
                );
        ...
        {
            DateTime appointmentStartWithRest = appointment.Starting.AddMinutes(
                -_settings.RestInMin
            );
        ...
        {
            var startTimes = timeIntervals
                .Where(x => x.From.Date == day.Date)
                .Select(x => x.From)
                .ToArray();
